...
func GenGraphCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "spograph",
		Short: "Draw a business architecture diagram based on the project created by the go development framework sponge",
		Long:  "Draw a business architecture diagram based on the project created by the go development framework sponge.",
  ...
  spograph --project-dir=/path/to/project

  # You can also specify multiple services to generate a business framework diagram
  spograph --server-dir=/path/to/server1 --server-dir=/path/to/server2

  # Includes database related servers
  spograph --project-dir=/path/to/project --all`),
		SilenceErrors: true,
		SilenceUsage:  true,
...
func getYamlFiles(projectDir string, serverDirs []string) []string {
	...
	configsDirs, _ := ListSubDirs(projectDir, "configs")
	yamlFiles = filterYamlFiles(configsDirs, yamlFiles)

	for _, dir := range serverDirs {
		configsDirs, _ = ListSubDirs(dir, "configs")
		yamlFiles = filterYamlFiles(configsDirs, yamlFiles)
	}
	return yamlFiles
}
	...
	for _, service := range pc.Services {
		...
		for typeLabel, dependencies := range service.Dependencies {
			for _, dependencyServiceName := range dependencies {
				key := sortAndMergeFields(service.Name, dependencyServiceName, typeLabel)
				if e, ok := edgeMap[key]; ok {
					setCustomEdgeStyle(e, typeLabel, getEdgeColor(typeLabel), true)
					continue
				}

				node2, _ := graph.CreateNode(dependencyServiceName)
				setCustomNodeStyle(node2, getNodeColor(dependencyServiceName))
